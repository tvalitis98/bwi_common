  ...
  *level_subscriber_ = n.subscribe<multi_level_map_msgs::LevelMetaData>("level_mux/current_level", 1, &levelHandler);
  ...
  while(ros::ok()) {
    ...
    if(curl) {
      ...
      std::string time_remaining_str = "time_remaining=" + boost::lexical_cast<std::string>(get_estimate(voltage));
      ...
      std::string params = name_str + "&" + timestamp_str + "&" + time_remaining_str + "&" + user_str + "&" + x_str + "&" + y_str;
